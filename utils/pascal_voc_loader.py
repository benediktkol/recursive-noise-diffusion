...
def decode_segmap(seg, is_one_hot=False, plot=False):
        """Decode segmentation class labels into a color image

        Args:
            seg (np.ndarray): an (M,N) array of integer values denoting
        ...
        seg = seg.type(dtype=torch.uint8)
        seg_img = torch.zeros((seg.shape[0], seg.shape[1], 3), dtype=torch.uint8)
        for c in range(22):
